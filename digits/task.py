...
########################################################
        ...
        #     pass
        # else:
        #     env['PYTHONPATH'] = os.pathsep.join(['.:/home/liubo/mxnet/python',  env.get('PYTHONPATH', '')])
        ...
        self.logger.info('args = %s' % ''.join(str(x) for x in args))
        # assert (0)

        # print env['PYTHONPATH']
        # print '-----------------------------------------++++++++++++++++++++++++++++++++++++++++++++++++'
        objlist = dir(self)
        if 'machine_select' in objlist:
            if(int(self.machine_select)==1 ):
                ...
                # assert (0)
            elif (int(self.machine_select)==2):
                self.p = subprocess.Popen(''.join(str(x) for x in args),
                                          shell=True,
                                          stdout=subprocess.PIPE,
                                          stderr=subprocess.STDOUT,
                                          cwd=self.job_dir,
