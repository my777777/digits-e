...
/////////////////////////////////////////
        $scope.get_selected_job_names = function () {
            var job_names = [];
            for(var i = 0; i<$scope.jobs.length; i++)
                if($scope.jobs[i].selected)
                    job_names.push($scope.jobs[i].name);
            return job_names;
        }
/////////////////////////////////////////
        ...
        //////////////////////////////////////////////////////////
        $scope.compare_jobs = function () {

            var job_ids = $scope.get_selected_job_ids();
            var job_names = $scope.get_selected_job_names();
            if (job_ids.length == 0) return;
            if (job_ids.length == 1 )
                bootbox.dialog({
                    title:"note",
                    message:"plz choose more than one job",
                    animate:true,
                    buttons:{
                        danger:{
                            label:"ok"
                ...
                window.open("http://222.20.79.231:9021/models/images/classification/showjobval?job_ids="+job_ids+"&job_names="+job_names);
        }
